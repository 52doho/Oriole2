...
- (void)downloadedVersionsData
{
    ...
    if (details)
    {
        ...
        if (showDetails && !self.visibleRemoteAlert)
        {
            ...
            [versionVC.view removeFromSuperview];
            versionVC = nil;
            
            UIViewController *rootViewController = [OOCommon getRootViewController];
            if (rootViewController && [self.delegate respondsToSelector:@selector(iVersionViewControllerForDisplay)])
            {
                versionVC = [self.delegate iVersionViewControllerForDisplay];
                NSAssert([versionVC isKindOfClass:[iVersionVC class]], @"View controller to show must be type of iVersionVC.");
            }
            if(rootViewController && versionVC)
            {
                UIView *addToView;
                ...
                versionVC.view.frame = realFrame;
                [addToView addSubview:versionVC.view];
                
                versionVC.lblTitle.text = title;
                versionVC.lblTitle.adjustsFontSizeToFitWidth = YES;
                versionVC.lblAppName.text = self.applicationName;
                versionVC.lblAppName.adjustsFontSizeToFitWidth = YES;
                versionVC.txtDesc.editable = NO;
                versionVC.txtDesc.text = details;
                OOLog(@"title:%@, details%@", title, details);
                [versionVC.btnUpdateNow setTitle:self.downloadButtonLabel forState:UIControlStateNormal];
                [versionVC.btnUpdateNow addTarget:self action:@selector(_btnUpgradeNowTapped) forControlEvents:UIControlEventTouchUpInside];
                [versionVC.btnRemindLater setTitle:self.remindButtonLabel forState:UIControlStateNormal];
                [versionVC.btnRemindLater addTarget:self action:@selector(_btnRemindLaterTapped) forControlEvents:UIControlEventTouchUpInside];
                [versionVC.btnNoUpdate setTitle:self.ignoreButtonLabel forState:UIControlStateNormal];
                [versionVC.btnNoUpdate addTarget:self action:@selector(_btnNoUpgradeTapped) forControlEvents:UIControlEventTouchUpInside];
                
                versionVC.bounceAnimationView.backgroundColor = [UIColor clearColor];
                [OOAnimation popUpAnimationForView:versionVC.bounceAnimationView];
            }
            else
            {
...
{
    return [NSString stringWithFormat:@"Trace_iVersion:%@", [OOCommon getAppVersion]];
}

#pragma mark - iVersionVC events -
- (void)_hideiVersionVC
{
    if(versionVC)
    {
        [UIView animateWithDuration:kDefaultAnimateDuration animations:^{
            versionVC.view.alpha = 0;
...
- (void)_btnUpgradeNowTapped
{
    [self _hideiVersionVC];
...
//    [Flurry logEvent:[self _getFlurryEventNameBaseOnVersion] withParameters:@{@"type": @"Download"}];
}
