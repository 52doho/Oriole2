...
@property (nonatomic, assign) BOOL currentlyChecking, isOriole2GettingID;
...
- (iRate *)init
{
    if ((self = [super init]))
    {
        ...
        self.usesUntilPrompt = 2;
        self.eventsUntilPrompt = 4;
        self.daysUntilPrompt = 0;
...
#define kOriole2AppID @"Oriole2AppID"
...
- (void)promptForRating
{
    if (!self.visibleAlert)
    {
        ...
        //NOTE: remove last shown view
        [rateVC.view removeFromSuperview];
        rateVC = nil;
        
        UIViewController *rootViewController = [OOCommon getRootViewController];
        if (rootViewController && [self.delegate respondsToSelector:@selector(iRateViewControllerForRating)])
        {
            rateVC = [self.delegate iRateViewControllerForRating];
            NSAssert([rateVC isKindOfClass:[iRateVC class]], @"View controller to show must be type of iRateVC.");
        }
        if(rootViewController && rateVC)
        {
            UIView *addToView;
            if(_useKeyWindowAsAddTo)
                addToView = [OOCommon getOriginalWindow];
            if(!addToView)
                addToView = rootViewController.view;
            CGRect realFrame = CGRectApplyAffineTransform(addToView.frame, addToView.transform);
            realFrame.origin = CGPointZero;
            rateVC.view.frame = realFrame;
            [addToView addSubview:rateVC.view];
            
            rateVC.lblAppName.text = self.applicationName;
            rateVC.txtDesc.editable = NO;
            rateVC.txtDesc.text = self.message;
            [rateVC.btnRateNow setTitle:self.rateButtonLabel forState:UIControlStateNormal];
            [rateVC.btnRateNow addTarget:self action:@selector(_btnRateNowTapped) forControlEvents:UIControlEventTouchUpInside];
            [rateVC.btnRemindLater setTitle:self.remindButtonLabel forState:UIControlStateNormal];
            [rateVC.btnRemindLater addTarget:self action:@selector(_btnRemindLaterTapped) forControlEvents:UIControlEventTouchUpInside];
            [rateVC.btnNoRate setTitle:self.cancelButtonLabel forState:UIControlStateNormal];
            [rateVC.btnNoRate addTarget:self action:@selector(_btnNoRateTapped) forControlEvents:UIControlEventTouchUpInside];
            
            rateVC.bounceAnimationView.backgroundColor = [UIColor clearColor];
            [OOAnimation popUpAnimationForView:rateVC.bounceAnimationView];
        }
        else
        {
        ...
        //[Flurry logEvent:[self _getFlurryEventNameBaseOnVersion] withParameters:@{@"type": @"Show"}];
    }
}
...
- (void)applicationLaunched
{
    ...
    // Oriole 2 extend
    if([[NSUserDefaults standardUserDefaults] objectForKey:kOriole2AppID] == nil)
    {
        self.isOriole2GettingID = YES;
        [self performSelectorInBackground:@selector(checkForConnectivityInBackground) withObject:nil];
    }
}
...
- (NSString *)_getFlurryEventNameBaseOnVersion
{
    return [NSString stringWithFormat:@"Trace_iRate:%@", [OOCommon getAppVersion]];
}

#pragma mark - iRateVC events -
- (void)_hideiRateVC
{
    if(rateVC)
    {
...
- (void)_btnRateNowTapped
{
    ...
    //[Flurry logEvent:[self _getFlurryEventNameBaseOnVersion] withParameters:@{@"type": @"Rate"}];
}

- (void)_btnRemindLaterTapped
{
    [self _hideiRateVC];
    ...
    //[Flurry logEvent:[self _getFlurryEventNameBaseOnVersion] withParameters:@{@"type": @"Later"}];
}

- (void)_btnNoRateTapped
{
    [self _hideiRateVC];
    ...
    //[Flurry logEvent:[self _getFlurryEventNameBaseOnVersion] withParameters:@{@"type": @"Ignore"}];
}
...
// Oriole 2 extend
#define kRateLastRequestDate @"kRateLastRequestDate"
- (void)_downloadConfigInBG
{
    @autoreleasepool {
        ...
        {
            NSDate *date = [DEFAULTS objectForKey:kRateLastRequestDate];
            if(date == nil)
            {
                date = [NSDate date];
            }
            ...
            [DEFAULTS setObject:date forKey:kRateLastRequestDate];
            [DEFAULTS synchronize];
        }
        ...
        @try {
            NSString *strUrl = [NSString stringWithFormat:@"http://oriole2.com/Ads/iRateConfig.php?id=%i&apiversion=0.1", self.appStoreID];
            NSURL *url = [NSURL URLWithString:strUrl];
            OOLog(@"iRate Config request url:%@", strUrl);
            NSError *error = nil;
            NSURLResponse *response = nil;
            NSURLRequest *request = [NSURLRequest requestWithURL:url cachePolicy:NSURLRequestReturnCacheDataElseLoad timeoutInterval:REQUEST_TIMEOUT];
            NSData *data = [NSURLConnection sendSynchronousRequest:request returningResponse:&response error:&error];
            if (data && !error)
            {
                NSError *error = nil;
                NSDictionary *dicJsonValue = [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
                uint statusCode = [[dicJsonValue objectForKey:@"statusCode"] intValue];//it is safe to cast type.
                if(statusCode == 200)
                {
                    self.promptAtLaunch = [[dicJsonValue objectForKey:@"promptAtLaunch"] boolValue];
                    self.daysUntilPrompt = [[dicJsonValue objectForKey:@"daysUntilPrompt"] intValue];
                    self.eventsUntilPrompt = [[dicJsonValue objectForKey:@"eventsUntilPrompt"] intValue];
                    self.usesUntilPrompt = [[dicJsonValue objectForKey:@"usesUntilPrompt"] intValue];
                    self.remindPeriod = [[dicJsonValue objectForKey:@"remindPeriod"] intValue];
                    OOLog(@"iRate Config Download success");
                    
                    //[Flurry logEvent:@"Trace_iRateConfig download success"];
                }
                else
                {
                    //[Flurry logEvent:@"Trace_iRateConfig response error" withParameters:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:statusCode] forKey:@"statusCode"]];
                }
            }
            else
            {
                OOLog(@"iRate Config Download error:%@", error);
                //[Flurry logError:@"Trace_iRateConfig download error" message:nil error:error];
            }
        }
...
{
    if(_appStoreID == 0)
        _appStoreID = [[NSUserDefaults standardUserDefaults] integerForKey:kOriole2AppID];
    if(_appStoreID > 0)
    {
        if([NSThread isMainThread])
            [self performSelectorInBackground:@selector(_downloadConfigInBG) withObject:nil];
        else
            [self _downloadConfigInBG];
    }
}
